...
# ---------- OWNER VIDEO LIST FUNCTION ----------
async def list_videos_for_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cursor.execute("SELECT code FROM films")
    films = cursor.fetchall()
    if not films:
        await update.callback_query.message.reply_text("Hozircha videolar yo‘q.")
        return

    for f in films:
        code = f[0]
        keyboard = [
            [InlineKeyboardButton("✏️ Kodni yangilash", callback_data=f"update_{code}"),
             InlineKeyboardButton("🗑 O‘chirish", callback_data=f"delete_{code}")]
        ]
        await update.callback_query.message.reply_text(f"Video kodi: {code}", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "upload_video":
        ...
        await list_videos_for_owner(update, context)
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        # Video kodi qo‘shish
        if action == "set_code" and update.message.video:
        ...
        if action == "broadcast":
            ...
            if update.message.video:
                ...
                caption = update.message.caption if update.message.caption else text
                for u in users:
                    try:
