...
async def handle_owner_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Agar action mavjud bo'lmasa yoki "upload_video" bo'lsa, avtomatik ravishda video yuklash rejimiga o'tkazish
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        ...
        if action == "set_code":
            ...
            # Video saqlandi, lekin context.user_data.clear() qilmaymiz
            # Buning o'rniga faqat video_file_id ni tozalaymiz
            # Shunday qilib keyingi video yuklash uchun tayyor bo'ladi
            context.user_data["video_file_id"] = None
            context.user_data["action"] = "upload_video"  # Keyingi video yuklash uchun tayyor
            return
        ...
        if action == "update_code":
            ...
            cursor.execute("SELECT * FROM films WHERE code=? AND code!=?", (new_code, old_code))
            if cursor.fetchone():
                await update.message.reply_text(f"⚠️ Bu kod band! Boshqa kod kiriting:")
                return
