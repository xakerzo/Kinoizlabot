...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        ...
        if action == "update_code":
            ...
            new_code = text
            cursor.execute("SELECT code FROM films WHERE code=?", (new_code,))
            if cursor.fetchone():
                await update.message.reply_text("Bu kod band! Iltimos, boshqa kod kiriting.")
                return
            cursor.execute("UPDATE films SET code=? WHERE code=?", (new_code, old_code))
            conn.commit()
            await update.message.reply_text(f"Kod muvaffaqiyatli alishtirildi: {old_code} → {new_code}")
            context.user_data.clear()
            return
    ...
    if result:
        ...
        if not_subscribed:
            keyboard = [[InlineKeyboardButton(f"✅ Obuna bo‘lish: {ch}", url=f"https://t.me/{ch[1:]}")]
